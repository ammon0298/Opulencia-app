...
const AuthView: React.FC<AuthViewProps> = ({ mode, error, successMessage, onLogin, onRegister, onBack, onSwitchMode, onRecoverInitiate, onRecoverVerify, onRecoverReset }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 md:p-6 selection:bg-indigo-500 selection:text-white">
                ...
                <form onSubmit={handleRecSubmit} className="space-y-6">
                    ...
                    {recoveryStep === 'code' && (
                        <div>
                            ...
                            <p className="text-[10px] text-center mt-4 text-slate-400">¿No recibió el código? <button type="button" onClick={() => setRecoveryStep('email')} className="text-indigo-600 font-bold hover:underline">Reenviar ahora</button></p>
                        </div>
                    )}
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className={`pt-4 ${mode === 'login' ? 'md:max-w-xl mx-auto' : ''}`}>
                        ...
                        {mode === 'login' && (
                            <button 
                                type="button" 
                                className="w-full mt-4 bg-slate-800 hover:bg-slate-900 text-white font-black py-4 md:py-5 rounded-2xl shadow-xl transition transform active:scale-95 uppercase tracking-[0.2em] text-[10px] md:text-xs border-b-4 border-slate-950"
                            >
                                hpt
                            </button>
                        )}
