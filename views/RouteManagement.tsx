    ...
    <div className="space-y-6 animate-fadeIn pb-10">
      <header className="flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-[2rem] border shadow-sm gap-4">
        ...
        <button 
          ...
          className={`px-8 py-3.5 rounded-2xl font-black transition shadow-xl flex items-center justify-center gap-2 active:scale-95 uppercase tracking-widest text-xs ${showForm ? 'bg-rose-100 text-rose-600' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}
        >
        ...
        <div className="bg-white p-10 rounded-[2rem] border shadow-xl animate-slideDown max-w-3xl mx-auto border-t-[8px] border-emerald-500">
           ...
           <form onSubmit={handleCreateRoute} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input type="text" value={newRouteName} onChange={e => setNewRouteName(e.target.value)} placeholder="Ej: Ruta Central" className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 focus:ring-4 focus:ring-indigo-100 outline-none transition font-black text-slate-800 shadow-inner" required />
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                    ...
                    <input type="number" value={initialBase} onChange={e => setInitialBase(e.target.value)} placeholder="0.00" className="w-full bg-slate-50 border border-slate-200 rounded-2xl pl-8 pr-4 py-4 focus:ring-4 focus:ring-emerald-100 outline-none transition font-black text-slate-800 shadow-inner" required />
                </div>
              </div>
      ...
      {editingRoute && (
          <div className="bg-white p-10 rounded-[2rem] border shadow-xl animate-slideDown border-t-[8px] border-indigo-600 max-w-4xl mx-auto">
              <div className="flex justify-between items-center mb-8">
                  <h3 className="text-2xl font-black text-slate-800">Gestionar: {editingRoute.name}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="space-y-6">
                      ...
                      <div className="bg-slate-50 rounded-2xl p-4 h-[300px] overflow-y-auto space-y-3 shadow-inner">
                          ...
                          )) : <p className="text-center py-20 text-[10px] font-black uppercase text-slate-300">Sin historial</p>}
                      </div>
                  </div>
              </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {routes.map(route => (
          <div key={route.id} className="bg-white p-8 rounded-[2.5rem] border shadow-sm hover:shadow-md transition flex flex-col min-h-[220px] group">
            <h3 className="font-black text-slate-800 text-2xl mb-1 group-hover:text-indigo-600 transition-colors">{route.name}</h3>
            ...
            <div className="mt-auto pt-6 flex gap-2">
                <button 
                    ...
                    className="flex-1 bg-slate-900 hover:bg-indigo-600 text-white px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md transition-all active:scale-95"
                >
                    Gestionar Fondos
                </button>
            </div>
